...
from brownie import TransparentUpgradeableProxyImpl, ProxyAdminImpl
from brownie import TestGas
from .utils import *
...
def check_token_prices(band_oracle, link_oracle, simple_oracle, agg_oracle, deployer):
    for idx, token in enumerate(tokens):
        ...
        try:
            out = simple_oracle.getPrice(token0, token1)
            print('out:', out)
            simple_price = to_float(out[0])
...
def check_low_gas(agg_oracle, deployer):
    test_oracle = TestGas.deploy(agg_oracle, {'from': deployer})
    ...
    for idx, token in enumerate(tokens):
        ...
        try:
            ...
            if token == INDEX:
                ...
                gases = [20000, 10000]
            print(test_oracle.getETHPx(token, gases))
        except Exception as e:
            print(e)
